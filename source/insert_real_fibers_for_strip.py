...
def main(parser):
    ...
    # extract meshname and the directory of mesh files (if None, take the current directory)
    ...
    R = np.load(R_path)
    print('Successfully read {}'.format(R_path))
    print('R.npy has shape (y,x,z):', R.shape)

    # ================================  PREPARE DATA ==========================================

    # extract quiver components
    fibres_YXZ = R['quiver_comp'][..., 0, :]  # -> shape: (Y, X, Z, 3)

    # Defines the voxel resolution of the data
    data_ps_yxz = np.array([0.439, 0.439, 2])  # um
    ...
    if _scaled:
        ...
        # the points coordinates have to be rescaled with a pixel size of 2 um
        mesh_ps_yxz = np.array([2, 2, 2])  # um
    ...
    print('MESH - Max coordinates of centroids (XYZ) [cpts]: ', np.array(cpts.max()))
    ...
    if _empty_fiber is False and _fake_fiber is False:
        print(
            BC.B + ' - Missing fibers will be replaced by the rule-based fiber (from the existent .lon file)' + BC.ENDC)

    if fiber_dir_vec is not None:
        print(BC.B + ' - Fibers versus will be realigned with axis {}: v='.format(fiber_direction), fiber_dir_vec,
              BC.ENDC)
    else:
    ...
    for i in range(n_points):
        ...
        # first check if the real fiber in this voxel of R is empty or discarded from analysis
        test = ~R[y, x, z]['freq_info']
        test2 = ~np.any(fibres_YXZ[y, x, z])
        if ~np.any(fibres_YXZ[y, x, z]) or ~R[y, x, z]['freq_info']:
            ...
            if _empty_fiber is True:
                ...
                # strip doesn't have rule-based fibers
                # lon_real.loc[i] = lon_rb.loc[i]  # fiber will be = to the rule-based
                lon_real.loc[i] = empty_fiber  # fiber will be = to the rule-based
            ...
            try:
                ...
                # normalize the lenght of the vector
                ev_xyz_norm = ev_xyz / np.linalg.norm(ev_xyz)

                # check normalization
                testnorm = np.linalg.norm(ev_xyz_norm)
                if np.linalg.norm(ev_xyz_norm) > 1.001:
                    print(BC.Y, 'Fail to normalize vector in R[{},{},{}]:'.format(y, x, z), BC.ENDC)
                ...
                if fiber_dir_vec is None:
                    ...
                    scalar = np.dot(ev_xyz_norm, np.array(lon_rb.loc[i]))
                else:
                    # alignment with user-selected direction
                    scalar = np.dot(ev_xyz_norm, fiber_dir_vec)

                # change the direction of the real versors is scalar < 0
                ev_xyz_final = ev_xyz_norm if scalar > 0 else np.negative(ev_xyz_norm)

                # finally insert real components
                lon_real.loc[i][[0, 1, 2]] = np.array(ev_xyz_final)  # already XYZ
    ...
    if _vtk:
        ...
        os.system('cd {0} && meshtool collect -imsh={1} -omsh={2} -fib={2}.lon -ofmt=vtu_bin'.format(
            os.path.dirname(mesh_basepath), mesh_basename, os.path.basename(real_lon_filename)))
...
if __name__ == '__main__':
    my_parser = argparse.ArgumentParser(
        description='Extract real fibers from R.npy OF STRIP and write into a .lon files')
