...
def main(parser):
    ...
    print(BC.Y + '*** Traslating the points along Z in the file: {} *** '.format(points_fname) + BC.ENDC)
    ...
    os.system('cd {0} && cp {1} {1}.backup_before_traslZ'.format(base_path, pts_filepath))
    print('Backup of the .pts file created as: {}.backup_before_traslZ'.format(points_fname))
    ...
    print(BC.B + 'Scale factor: ' + BC.ENDC, trasl)
    print('Start traslating the points of the mesh... ')
...
if __name__ == '__main__':
    my_parser = argparse.ArgumentParser(description='Load the mesh and add a translation along the z axis. \n Create a new .pts file and backup the old one.')
    ...
    my_parser.add_argument('-t',
                           '--trasl',
                           help='Translation (it will be ADDED to the z coordinates, assumed in MICRONS)',
                           required=True,
                           type=int)
    # run main
    main(my_parser)
